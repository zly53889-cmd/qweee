...
let airspaceLayerGroup: L.LayerGroup | null = null;
let protectionLayerGroup: L.LayerGroup | null = null;
let regionBoundaryLayer: L.LayerGroup | null = null;
let tianjinBoundaryLayer: L.Layer | null = null;
...
const updateAirspace = () => {
  if (!map || !airspaceLayerGroup) return;
  airspaceLayerGroup.clearLayers();

  if (props.showAirspace) {
    const sector1: [number, number][] = [[39.5, 117.0], [39.7, 117.4], [39.4, 117.6], [39.2, 117.2]];
    const sector2: [number, number][] = [[39.1, 117.4], [39.3, 117.8], [38.9, 117.9], [38.8, 117.5]];
    const sector3: [number, number][] = [[39.0, 116.8], [39.3, 117.0], [39.1, 117.3], [38.7, 117.1]];
    ...
    L.polygon(sector1, {
      ...
      fillColor: '#c084fc',
      fillOpacity: 0.1
    }).addTo(airspaceLayerGroup).bindTooltip("<span style='color:#c084fc;font-weight:bold;font-family:monospace;'>空域 A区</span>", { permanent: true, direction: 'center', className: 'bg-black/80 border border-[#c084fc]/30 px-1 py-0.5 rounded text-[9px] text-[#c084fc]' });
    ...
    L.polygon(sector2, {
      ...
      fillColor: '#60a5fa',
      fillOpacity: 0.1
    }).addTo(airspaceLayerGroup).bindTooltip("<span style='color:#60a5fa;font-weight:bold;font-family:monospace;'>空域 B区</span>", { permanent: true, direction: 'center', className: 'bg-black/80 border border-[#60a5fa]/30 px-1 py-0.5 rounded text-[9px] text-[#60a5fa]' });
    ...
    L.polygon(sector3, {
      ...
      fillColor: '#f43f5e',
      fillOpacity: 0.1
    }).addTo(airspaceLayerGroup).bindTooltip("<span style='color:#f43f5e;font-weight:bold;font-family:monospace;'>空域 C区</span>", { permanent: true, direction: 'center', className: 'bg-black/80 border border-[#f43f5e]/30 px-1 py-0.5 rounded text-[9px] text-[#f43f5e]' });
  }
};

const updateProtection = () => {
  if (!map || !protectionLayerGroup) return;
  protectionLayerGroup.clearLayers();

  if (props.showProtection) {
    const center: [number, number] = [39.13, 117.20];
    ...
    // Labels nested
    const labelPos30: [number, number] = [39.13 + 0.27, 117.20];
    const labelPos60: [number, number] = [39.13 + 0.54, 117.20];

    L.marker(labelPos30, {
      icon: L.divIcon({
        html: `<div class="text-[#00d0ff] text-[8px] font-bold font-mono tracking-wider bg-[#050b1c]/95 border border-[#00d0ff]/40 px-1.5 py-0.5 rounded shadow-[0_0_8px_rgba(0,208,255,0.4)] whitespace-nowrap">30km 核心保障圈</div>`,
        className: '',
        iconSize: [80, 16],
        iconAnchor: [40, 8]
      })
    ...
    L.marker(labelPos60, {
      icon: L.divIcon({
        html: `<div class="text-[#00d0ff] text-[8px] font-bold font-mono tracking-wider bg-[#050b1c]/95 border border-[#00d0ff]/40 px-1.5 py-0.5 rounded shadow-[0_0_8px_rgba(0,208,255,0.4)] whitespace-nowrap">60km 协同保障圈</div>`,
        className: '',
        iconSize: [80, 16],
        iconAnchor: [40, 8]
      })
...
const updateRegion = async () => {
  if (!map || !regionBoundaryLayer) return;
  regionBoundaryLayer.clearLayers();

  if (props.region === 'huabei') {
    map.flyTo([38.8, 116.2], 7, { animate: true, duration: 1.5 });

    const urls = [
      { code: '110000', name: '北京', color: '#3b82f6' },
      { code: '130000', name: '河北', color: '#10b981' }
    ];

    for (const item of urls) {
      try {
        const res = await fetch(`https://geo.datav.aliyun.com/areas_v3/bound/${item.code}_full.json`);
        if (res.ok) {
        ...
        console.warn(`Failed to fetch ${item.name} boundaries`, e);
      }
    }
  } else {
    map.flyTo([39.13, 117.20], 9, { animate: true, duration: 1.5 });
  }
};
...
onMounted(() => {
  ...
  protectionLayerGroup = L.layerGroup().addTo(map);
  regionBoundaryLayer = L.layerGroup().addTo(map);
  ...
  watch(() => props.showAirspace, () => {
    updateAirspace();
  });

  watch(() => props.showProtection, () => {
    updateProtection();
  });
