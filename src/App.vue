...
const currentMapRegion = ref<'tianjin' | 'huabei'>('tianjin');
const showAirspacePartition = ref(false);
const showProtectionCircles = ref(false);

const isOperationExpanded = ref(true);
const isProductExpanded = ref(true);

const toggleOperationExpanded = () => {
  isOperationExpanded.value = !isOperationExpanded.value;
};
          ...
          <section class="flex-1 flex flex-col h-full overflow-hidden">
             <div class="card flex-1 p-0.5 bg-[#050b1c]/80 flex flex-col overflow-hidden relative border border-[#2d7ad4]/30 shadow-[inset_0_0_30px_rgba(45,122,212,0.1)]">
                <div class="w-full h-full relative overflow-hidden rounded-[2px]">
                   <LeafletMap :show-fixed="showFixedPoints" :show-mobile="showMobilePoints" :show-personnel="showPersonnel" :overlays="overlayOptions" :region="currentMapRegion" :show-airspace="showAirspacePartition" :show-protection="showProtectionCircles" />
                   ...
                   <!-- AI Chat Dialog -->
                   <div v-if="isChatOpen" class="absolute top-24 right-6 w-[350px] max-h-[calc(100%-120px)] h-[520px] z-[502] flex flex-col bg-[#050b1c]/95 border border-[#00d0ff]/30 rounded-lg shadow-[0_10px_40px_rgba(0,0,0,0.8)] backdrop-blur-xl overflow-hidden animate-in fade-in zoom-in duration-300">
                      <!-- Chat Header -->
                   ...
                   <div class="absolute top-6 left-6 flex flex-col gap-3 z-[500] max-h-[calc(100%-250px)] overflow-y-auto custom-scrollbar pr-1.5 pb-2">
                      ...
                      <div class="w-[180px] p-4 backdrop-blur-md border border-[#2d7ad4]/30 rounded-sm overflow-hidden" style="background: linear-gradient(135deg, rgba(10, 35, 79, 0.9), rgba(3, 13, 36, 0.9)); box-shadow: 0 8px 32px rgba(0,0,0,0.5);">
                         <!-- Header -->
                         <div @click="toggleOperationExpanded" class="flex items-center justify-between mb-2 cursor-pointer group/header">
                            <div class="flex items-center gap-2">
                      ...
                      <div class="w-[180px] p-4 backdrop-blur-md border border-[#2d7ad4]/30 rounded-sm overflow-hidden" style="background: linear-gradient(135deg, rgba(10, 35, 79, 0.9), rgba(3, 13, 36, 0.9)); box-shadow: 0 8px 32px rgba(0,0,0,0.5);">
                         <!-- Header -->
                         <div @click="toggleProductExpanded" class="flex items-center justify-between mb-3 cursor-pointer group/head">
                            <div class="flex items-center gap-2">
                   ...
                   <!-- Map and Product Legends (Left Aligned & Stacked) -->
                   <div class="absolute bottom-24 left-6 flex flex-col gap-2 z-[500] items-start">
                      <!-- Map Legend from Subsystem -->
                      <div class="w-[105px] border border-[#2d7ad4]/50 rounded-sm p-2 backdrop-blur-md" style="background: radial-gradient(circle at 100% 0%, rgba(10, 35, 79, 0.9), rgba(3, 13, 36, 0.9)); box-shadow: inset 0 0 15px rgba(0,208,255,0.05), 0 4px 20px rgba(0,0,0,0.6);">
                          <div class="text-[#64a9f2] text-[11px] font-bold pb-1.5 flex items-center justify-center border-b border-[#2d7ad4]/40 mb-1.5 tracking-widest">
                            图例
                          </div>
                          <ul class="space-y-1.5 pt-0.5 text-[10px] text-[#a5d8ff]">
                            <li class="flex items-center justify-between cursor-pointer hover:text-white group transition-colors">
                              ...
                              <div class="w-1.5 h-1.5 rounded-full border border-[#ff9100] bg-[#ff9100] shadow-[0_0_6px_#ff9100]"></div>
                            </li>
                            <li class="flex items-center justify-between cursor-pointer hover:text-white group transition-colors">
                              <span class="group-hover:translate-x-0.5 transition-transform">作业中</span>
                              <div class="w-1.5 h-1.5 rounded-full border border-[#2dcc5f] bg-[#2dcc5f] shadow-[0_0_6px_#2dcc5f]"></div>
                            </li>
                            <li class="flex items-center justify-between cursor-pointer hover:text-white group transition-colors">
                              <span class="group-hover:translate-x-0.5 transition-transform">已完成</span>
                              <div class="w-1.5 h-1.5 rounded-full border border-[#ff4747] bg-[#ff4747] shadow-[0_0_6px_#ff4747]"></div>
                            </li>
                      ...
                      <div v-if="isAnyOverlayActive" class="w-[105px] py-1.5 px-2 backdrop-blur-md border border-[#2d7ad4]/30 rounded-sm animate-in fade-in slide-in-from-left-4 duration-500 flex flex-col gap-0.5" style="background: rgba(10, 35, 79, 0.95);">
                         <div class="text-[9px] text-[#a5d8ff]/90 font-bold tracking-wider text-center leading-tight">{{ activeOverlayLabel }}</div>
                         <div class="h-1 w-full bg-gradient-to-r from-[#2dcc5f] via-[#ffe600] to-[#ff4747] rounded-full"></div>
                         <div class="flex justify-between px-0.5 text-[7px] text-[#a5d8ff]/60 leading-none">
                            <span>15</span>
                            <span>35</span>
                            <span>55</span>
                            <span>75+</span>
                   ...
                   <!-- Optional Overlay for tech feel -->
                    <!-- Bottom Right Map Control Panel (地图切换) -->
                    <div class="absolute bottom-24 right-6 w-[105px] border border-[#2d7ad4]/50 rounded-sm p-2.5 backdrop-blur-md z-[501]" style="background: radial-gradient(circle at 100% 0%, rgba(10, 35, 79, 0.95), rgba(3, 13, 36, 0.95)); box-shadow: inset 0 0 15px rgba(0,208,255,0.05), 0 4px 20px rgba(0,0,0,0.6);">
                        <div class="text-[#64a9f2] text-[11px] font-bold pb-1.5 border-b border-[#2d7ad4]/40 mb-2 tracking-widest text-center leading-none">
                          地图切换
                        </div>
                        ...
                        <div class="space-y-2 pt-0.5">
                          <!-- 天津区域 checkbox -->
                          <label class="flex items-center gap-1.5 py-0.5 cursor-pointer group pointer-events-auto leading-none">
                            <div class="relative w-3.5 h-3.5 flex items-center justify-center shrink-0">
                               <input type="checkbox" :checked="currentMapRegion === 'tianjin'" @change="currentMapRegion = 'tianjin'" class="peer appearance-none w-3.5 h-3.5 border border-[#00d0ff] rounded-sm bg-[#00d0ff]/10 checked:bg-[#00d0ff]/40 transition-all cursor-pointer" />
                               <div class="absolute inset-[3px] flex items-center justify-center text-[#00d0ff] opacity-0 peer-checked:opacity-100 pointer-events-none rounded-[1px] bg-[#00d0ff] shadow-[0_0_6px_#00d0ff]"></div>
                            </div>
                            <span class="text-[#a5d8ff] text-[10px] group-hover:text-white transition-colors">天津区域</span>
                          </label>

                          <!-- 华北区域 checkbox -->
                          <label class="flex items-center gap-1.5 py-0.5 cursor-pointer group pointer-events-auto leading-none">
                            <div class="relative w-3.5 h-3.5 flex items-center justify-center shrink-0">
                               <input type="checkbox" :checked="currentMapRegion === 'huabei'" @change="currentMapRegion = 'huabei'" class="peer appearance-none w-3.5 h-3.5 border border-[#00d0ff] rounded-sm bg-[#00d0ff]/10 checked:bg-[#00d0ff]/40 transition-all cursor-pointer" />
                               <div class="absolute inset-[3px] flex items-center justify-center text-[#00d0ff] opacity-0 peer-checked:opacity-100 pointer-events-none rounded-[1px] bg-[#00d0ff] shadow-[0_0_6px_#00d0ff]"></div>
                            </div>
                            <span class="text-[#a5d8ff] text-[10px] group-hover:text-white transition-colors">华北区域</span>
                          </label>

                          <!-- 空域分区 checkbox -->
                          <label class="flex items-center gap-1.5 py-0.5 cursor-pointer group pointer-events-auto leading-none">
                            <div class="relative w-3.5 h-3.5 flex items-center justify-center shrink-0">
                               <input type="checkbox" v-model="showAirspacePartition" class="peer appearance-none w-3.5 h-3.5 border border-[#c084fc] rounded-sm bg-[#c084fc]/10 checked:bg-[#c084fc]/40 transition-all cursor-pointer" />
                               <div class="absolute inset-[3px] flex items-center justify-center text-[#c084fc] opacity-0 peer-checked:opacity-100 pointer-events-none rounded-[1px] bg-[#c084fc] shadow-[0_0_6px_#c084fc]"></div>
                            </div>
                            <span class="text-[#a5d8ff] text-[10px] group-hover:text-white transition-colors">空域分区</span>
                          </label>
                          
                          <!-- 保障圈 checkbox -->
                          <label class="flex items-center gap-1.5 py-0.5 cursor-pointer group pointer-events-auto leading-none">
                            <div class="relative w-3.5 h-3.5 flex items-center justify-center shrink-0">
                               <input type="checkbox" v-model="showProtectionCircles" class="peer appearance-none w-3.5 h-3.5 border border-[#00d0ff] rounded-sm bg-[#00d0ff]/10 checked:bg-[#00d0ff]/40 transition-all cursor-pointer" />
                               <div class="absolute inset-[3px] flex items-center justify-center text-[#00d0ff] opacity-0 peer-checked:opacity-100 pointer-events-none rounded-[1px] bg-[#00d0ff] shadow-[0_0_6px_#00d0ff]"></div>
                            </div>
                            <span class="text-[#a5d8ff] text-[10px] group-hover:text-white transition-colors">保障圈</span>
                          </label>
                        </div>
                    </div>
